...
export class ReportsComponent implements OnInit {
  ...
  loadStats() {
    this.dashboardService.getStats().subscribe({
      ...
      error: (err) => { console.error(err); this.toastService.error('Failed to load statistics'); }
    });
  }
  ...
  exportToExcel() {
    this.dashboardService.exportToExcel().subscribe({
      next: (blob) => {
        ...
        a.href = url; a.download = `inventory-export-${new Date().getTime()}.xlsx`; a.click();
        window.URL.revokeObjectURL(url);
        this.toastService.success('Excel export downloaded successfully');
      },
      error: (err) => { console.error(err); this.toastService.error('Failed to export to Excel'); }
    });
  }

  syncWithTally() {
    this.dashboardService.syncWithTally().subscribe({
      next: (response) => { this.toastService.success(response.message || 'Tally sync completed successfully'); this.loadStats(); },
      error: (err) => { console.error(err); this.toastService.error('Failed to sync with Tally'); }
    });
  }
